...
function genClientList() {
	var previousClientList = clientList;
	clientList = [];
	...
	var isValidMacKey = function (key) {
		return /^[0-9A-F]{2}(:[0-9A-F]{2}){5}$/i.test(key);
	};
	var normalizeClientBucket = function (bucket) {
		if (!bucket || typeof bucket != "object") {
			return { maclist: [] };
		}

		if (!Array.isArray(bucket.maclist)) {
			bucket.maclist = Object.keys(bucket).filter(function (key) {
				return key != "maclist" &&
					key != "ClientAPILevel" &&
					isValidMacKey(key) &&
					typeof bucket[key] == "object";
			});
		}

		return bucket;
	};
	var getCachedClient = function (mac) {
		if (!previousClientList || typeof previousClientList[mac] == "undefined") {
			return null;
		}
		return previousClientList[mac];
	};
	...
	var resolveClientText = function (value, cachedValue, defaultValue) {
		var result = normalizeTextValue(value);
		if (result !== "") {
			return result;
		}
		var cachedResult = normalizeTextValue(cachedValue);
		if (cachedResult !== "") {
			return cachedResult;
		}
		return defaultValue;
	};
	var isClientOnline = function (client) {
		if (!client || typeof client != "object") {
			return false;
		}
		return (client.isOnline == "1" || client.online == "1" || client.isOnline === true || client.online === true);
	};
	var parseConnTs = function (value) {
		var connTs = parseInt(value, 10);
		if (isNaN(connTs) || connTs <= 0) {
			return 0;
		}
		return connTs;
	};
	...
	var resolveConnectTime = function (client, cachedClient, isOnline, hasNetworkmapdData) {
		var resolved = resolveClientText(client.wlConnectTime, (cachedClient ? cachedClient.wlConnectTime : ""), "");
		if (resolved !== "") {
			return resolved;
		}

		// Only compute uptime from conn_ts if the client has actual networkmapd data.
		// nmpClient's conn_ts is a "last seen" timestamp, not a "connected since" timestamp,
		// so using it to compute uptime would produce absurdly large durations.
		if (hasNetworkmapdData) {
			var connTs = parseConnTs(client.conn_ts);
			var nowTs = Math.floor(Date.now() / 1000);
			if (isOnline && connTs > 0 && nowTs >= connTs) {
				return secondsToClock(nowTs - connTs);
			}
		}

		return "00:00:00";
	};
	var resolveClientIp = function (client, cachedClient, isOnline) {
		var resolved = resolveClientText(client.ip, (cachedClient ? cachedClient.ip : ""), "");
		if (resolved !== "") {
			if (resolved == "offline" && isOnline) {
				return "0.0.0.0";
			}
			if (resolved == "0.0.0.0" && !isOnline) {
				return "offline";
			}
			return resolved;
		}
		return isOnline ? "0.0.0.0" : "offline";
	};
	var resolveIsWL = function (client, cachedClient) {
		var isWL = parseInt((typeof client.isWL == "undefined") ? client.wireless : client.isWL, 10);
		if (isNaN(isWL) && cachedClient) {
			isWL = parseInt(cachedClient.isWL, 10);
		}
		return isNaN(isWL) ? 0 : isWL;
	};
	var resolveIsWireless = function (client, cachedClient, isWL) {
		var isWireless = parseInt((typeof client.is_wireless == "undefined") ? client.wireless : client.is_wireless, 10);
		if (isNaN(isWireless) && cachedClient) {
			isWireless = parseInt(cachedClient.is_wireless, 10);
		}
		if (isNaN(isWireless)) {
			isWireless = (isWL > 0) ? 1 : 0;
		}
		return isWireless;
	};
	var fromNetworkmapdBucket = { maclist: [] };
	var nmpClientBucket = { maclist: [] };
	if (originData.fromNetworkmapd != undefined && originData.fromNetworkmapd.length > 0) {
		fromNetworkmapdBucket = normalizeClientBucket(originData.fromNetworkmapd[0]);
	}
	if (originData.nmpClient != undefined && originData.nmpClient.length > 0) {
		nmpClientBucket = normalizeClientBucket(originData.nmpClient[0]);
	}
	...
	if (Array.isArray(fromNetworkmapdBucket.maclist)) {
		for (var i = 0; i < fromNetworkmapdBucket.maclist.length; i++) {
			var thisClient = fromNetworkmapdBucket[fromNetworkmapdBucket.maclist[i]];
			...
			var cachedClient = getCachedClient(thisClientMacAddr);
			clientList[thisClientMacAddr].isOnline = isClientOnline(thisClient);
			if (clientList[thisClientMacAddr].isOnline)
				totalClientNum.online++;
			...
			clientList[thisClientMacAddr].ip = resolveClientIp(thisClient, cachedClient, clientList[thisClientMacAddr].isOnline);
			clientList[thisClientMacAddr].ip6 = resolveClientText(thisClient.ip6, (cachedClient ? cachedClient.ip6 : ""), "");
			clientList[thisClientMacAddr].ip6_prefix = resolveClientText(thisClient.ip6_prefix, (cachedClient ? cachedClient.ip6_prefix : ""), "");
			clientList[thisClientMacAddr].conn_ts = parseConnTs(thisClient.conn_ts);
			if (clientList[thisClientMacAddr].conn_ts == 0 && cachedClient) {
				clientList[thisClientMacAddr].conn_ts = parseConnTs(cachedClient.conn_ts);
			}
			clientList[thisClientMacAddr].mac = thisClient.mac.toUpperCase();

			clientList[thisClientMacAddr].name = resolveClientText(thisClient.name, (cachedClient ? cachedClient.name : ""), "");
			if (clientList[thisClientMacAddr].name == "") {
				clientList[thisClientMacAddr].name = clientList[thisClientMacAddr].mac;
			}
			clientList[thisClientMacAddr].nickName = resolveClientText(thisClient.nickName, (cachedClient ? cachedClient.nickName : ""), "");
			clientList[thisClientMacAddr].hostname = resolveClientText(thisClient.hostname, (cachedClient ? cachedClient.hostname : ""), "");
			...
			clientList[thisClientMacAddr].vendor = (typeof thisClient.vendor == "string") ? thisClient.vendor : "";
			clientList[thisClientMacAddr].rssi = parseInt(thisClient.rssi);
			if (isNaN(clientList[thisClientMacAddr].rssi) && cachedClient) {
				clientList[thisClientMacAddr].rssi = parseInt(cachedClient.rssi);
			}
			if (isNaN(clientList[thisClientMacAddr].rssi)) {
				clientList[thisClientMacAddr].rssi = "";
			}
			clientList[thisClientMacAddr].isWL = resolveIsWL(thisClient, cachedClient);
			clientList[thisClientMacAddr].is_wireless = resolveIsWireless(thisClient, cachedClient, clientList[thisClientMacAddr].isWL);
			clientList[thisClientMacAddr].sdn_idx = parseInt(thisClient.sdn_idx, 10);
			if (isNaN(clientList[thisClientMacAddr].sdn_idx)) {
				clientList[thisClientMacAddr].sdn_idx = (cachedClient && !isNaN(parseInt(cachedClient.sdn_idx, 10))) ? parseInt(cachedClient.sdn_idx, 10) : 0;
			}
			clientList[thisClientMacAddr].sdn_type = thisClient.sdn_type || "";
			...
			if (isSupport("stainfo")) {
				clientList[thisClientMacAddr].curTx = resolveClientText(thisClient.curTx, (cachedClient ? cachedClient.curTx : ""), "");
				clientList[thisClientMacAddr].curRx = resolveClientText(thisClient.curRx, (cachedClient ? cachedClient.curRx : ""), "");
				clientList[thisClientMacAddr].wlConnectTime = resolveConnectTime(thisClient, cachedClient, clientList[thisClientMacAddr].isOnline, true);
			}
	...
	var nmpCount = 0;
	if (Array.isArray(nmpClientBucket.maclist)) {
		for (var i = 0; i < nmpClientBucket.maclist.length; i++) {
			var thisClient = nmpClientBucket[nmpClientBucket.maclist[i]];
			var thisClientMacAddr = (typeof thisClient.mac == "undefined") ? false : thisClient.mac.toUpperCase();
			if (!thisClientMacAddr) {
				continue;
			}
			...
			var cachedClient = getCachedClient(thisClientMacAddr);
			// nmpClient's online status from JSON database can be stale because the
			// networkmap daemon doesn't always sync offline transitions back to the DB.
			// fromNetworkmapd (shared memory) is the authoritative source for online status.
			var thisClientOnline = (typeof clientList[thisClientMacAddr] !== "undefined") ?
				clientList[thisClientMacAddr].isOnline : false;
			if (typeof clientList[thisClientMacAddr] == "undefined") {
				var thisClientType = resolveClientText(thisClient.type, (cachedClient ? cachedClient.type : ""), "0");
				var thisClientDefaultType = resolveClientText(thisClient.defaultType, (cachedClient ? cachedClient.defaultType : ""), thisClientType);
				var thisClientName = resolveClientText(thisClient.name, (cachedClient ? cachedClient.name : ""), thisClientMacAddr);
				if (thisClientName == "") {
					thisClientName = thisClientMacAddr;
				}
				thisClientName = htmlEnDeCode.htmlEncode(thisClientName);
				var thisClientNickName = resolveClientText(thisClient.nickName, (cachedClient ? cachedClient.nickName : ""), "");
				thisClientNickName = htmlEnDeCode.htmlEncode(thisClientNickName);
				var thisClientReNode = (typeof thisClient.amesh_isRe == "undefined")
					? ((cachedClient && cachedClient.amesh_isRe) ? true : false)
					: ((thisClient.amesh_isRe == "1") ? true : false);

				clientList.push(thisClientMacAddr);
				clientList[thisClientMacAddr] = new setClientAttr();
				clientList[thisClientMacAddr].from = thisClient.from || "nmpClient";
				if (!isSupport("sfp4m")) {
				...
				clientList[thisClientMacAddr].hostname = resolveClientText(thisClient.hostname, (cachedClient ? cachedClient.hostname : ""), "");
				clientList[thisClientMacAddr].vendor = resolveClientText(thisClient.vendor, (cachedClient ? cachedClient.vendor : ""), "");
				clientList[thisClientMacAddr].isOnline = thisClientOnline;
				clientList[thisClientMacAddr].ip = resolveClientIp(thisClient, cachedClient, thisClientOnline);
				clientList[thisClientMacAddr].ip6 = resolveClientText(thisClient.ip6, (cachedClient ? cachedClient.ip6 : ""), "");
				clientList[thisClientMacAddr].ip6_prefix = resolveClientText(thisClient.ip6_prefix, (cachedClient ? cachedClient.ip6_prefix : ""), "");
				clientList[thisClientMacAddr].conn_ts = parseConnTs(thisClient.conn_ts);
				if (clientList[thisClientMacAddr].conn_ts == 0 && cachedClient) {
				...
				clientList[thisClientMacAddr].curTx = resolveClientText(thisClient.curTx, (cachedClient ? cachedClient.curTx : ""), "");
				clientList[thisClientMacAddr].curRx = resolveClientText(thisClient.curRx, (cachedClient ? cachedClient.curRx : ""), "");
				clientList[thisClientMacAddr].wlConnectTime = resolveConnectTime(thisClient, cachedClient, thisClientOnline, false);
				clientList[thisClientMacAddr].rssi = parseInt(thisClient.rssi, 10);
				if (isNaN(clientList[thisClientMacAddr].rssi) && cachedClient) {
					clientList[thisClientMacAddr].rssi = parseInt(cachedClient.rssi, 10);
				}
				if (isNaN(clientList[thisClientMacAddr].rssi)) {
					clientList[thisClientMacAddr].rssi = "";
				}
				if (clientList[thisClientMacAddr].isOnline) {
					if (clientList[thisClientMacAddr].isWL > 0) {
						totalClientNum.wireless += clientList[thisClientMacAddr].macRepeat;
						if (totalClientNum.wireless_ifnames[clientList[thisClientMacAddr].isWL - 1] != undefined)
							totalClientNum.wireless_ifnames[clientList[thisClientMacAddr].isWL - 1] += clientList[thisClientMacAddr].macRepeat;
					}
					else {
						totalClientNum.wired += clientList[thisClientMacAddr].macRepeat;
				...
				clientList[thisClientMacAddr].from = thisClient.from || clientList[thisClientMacAddr].from || "nmpClient";
				if (clientList[thisClientMacAddr].hostname == "") {
					clientList[thisClientMacAddr].hostname = resolveClientText(thisClient.hostname, (cachedClient ? cachedClient.hostname : ""), "");
				}
				if (clientList[thisClientMacAddr].ip6 == "") {
					clientList[thisClientMacAddr].ip6 = resolveClientText(thisClient.ip6, (cachedClient ? cachedClient.ip6 : ""), "");
				}
				if (clientList[thisClientMacAddr].ip6_prefix == "") {
					clientList[thisClientMacAddr].ip6_prefix = resolveClientText(thisClient.ip6_prefix, (cachedClient ? cachedClient.ip6_prefix : ""), "");
				}
				if (clientList[thisClientMacAddr].curTx == "") {
					clientList[thisClientMacAddr].curTx = resolveClientText(thisClient.curTx, (cachedClient ? cachedClient.curTx : ""), "");
				}
				if (clientList[thisClientMacAddr].curRx == "") {
					clientList[thisClientMacAddr].curRx = resolveClientText(thisClient.curRx, (cachedClient ? cachedClient.curRx : ""), "");
				}
				if (clientList[thisClientMacAddr].wlConnectTime == "" || clientList[thisClientMacAddr].wlConnectTime == "00:00:00") {
					clientList[thisClientMacAddr].wlConnectTime = resolveConnectTime(thisClient, cachedClient, (clientList[thisClientMacAddr].isOnline || thisClientOnline), false);
				}

				var nmpConnTs = parseConnTs(thisClient.conn_ts);
				if (nmpConnTs > 0) {
					clientList[thisClientMacAddr].conn_ts = nmpConnTs;
				}

				if (!clientList[thisClientMacAddr].isOnline && thisClientOnline) {
					clientList[thisClientMacAddr].isOnline = true;
					clientList[thisClientMacAddr].ip = resolveClientIp(thisClient, clientList[thisClientMacAddr], true);
					clientList[thisClientMacAddr].ip6 = resolveClientText(thisClient.ip6, clientList[thisClientMacAddr].ip6, "");
					clientList[thisClientMacAddr].ip6_prefix = resolveClientText(thisClient.ip6_prefix, clientList[thisClientMacAddr].ip6_prefix, "");
					clientList[thisClientMacAddr].isWL = resolveIsWL(thisClient, clientList[thisClientMacAddr]);
					clientList[thisClientMacAddr].is_wireless = resolveIsWireless(thisClient, clientList[thisClientMacAddr], clientList[thisClientMacAddr].isWL);
					clientList[thisClientMacAddr].curTx = resolveClientText(thisClient.curTx, clientList[thisClientMacAddr].curTx, "");
					clientList[thisClientMacAddr].curRx = resolveClientText(thisClient.curRx, clientList[thisClientMacAddr].curRx, "");
					clientList[thisClientMacAddr].wlConnectTime = resolveConnectTime(thisClient, clientList[thisClientMacAddr], true, false);
					totalClientNum.online++;
					if (clientList[thisClientMacAddr].isWL > 0) {
						totalClientNum.wireless += clientList[thisClientMacAddr].macRepeat;
						if (totalClientNum.wireless_ifnames[clientList[thisClientMacAddr].isWL - 1] != undefined)
...
function _buildClientListRowHtml(profileArray, objID) {
	...
	const hasIpv4 = (p.ip != "" && p.ip != "0.0.0.0" && p.ip != "offline");
	const hasIpv6 = (p.ip6 != undefined && p.ip6 != "");
	const clientIpGroupCode = hasIpv4 ? `<div>${p.ip}</div>` : (hasIpv6 ? `<div>${p.ip6}</div>` : `<div>-</div>`);
	const clientIpLinkCode = hasIpv4 ? `${p.ip}` : (hasIpv6 ? `[${p.ip6}]` : "");
	let clientIpExtraCode = ``;
	if (p.ip6_prefix != undefined && p.ip6_prefix !== "") {
		clientIpExtraCode += `<div style="font-size: 0.75em;" title="<#IPv6_wan_addr#>">${p.ip6_prefix}</div>`;
	}
	if (hasIpv4 && hasIpv6) {
		clientIpExtraCode += `<div style="font-size: 0.75em;" title="WAN IPv6 Link-Local">${p.ip6}</div>`;
	}
	const clientIpGroupDetailCode = `<div class="client-ip-group">${clientIpGroupCode}${clientIpExtraCode}</div>`;
				        ...
				        <div class='client_info_main'>
				            <div class='client_info_text'>
				                <div class='client-name'>
                                    ...
                                    <div class='client-ip'>`;
	rowHtml += (isWebServer && clientIpLinkCode != "")
		? `<a class='link' href='http://${clientIpLinkCode}' target='_blank'>${clientIpGroupDetailCode}</a>`
		: clientIpGroupDetailCode;
